...
import { TransportCancelProcessor } from "./TransportCancelProcessor";
import { validateTransportsCancel } from '../common/input';
...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    try {
        ...
        const transportProcessor = new TransportCancelProcessor(apiClient, logger, inputParameters.environment);

        const request = await transportProcessor.getTransport(_id);

        if (request && !request.mmm_ata && !request.mmm_atd) {
            const validateFields = validateTransportsCancel(newTransport, context);
            if (validateFields === false) {
                context.done();
                return;
            }
            
            const result = await transportProcessor.updateTransportCancel(_id, newTransport);
            ...
            context.log.warn(`Item with id: ${_id} already contains ATA or ATD`);
            context.res = {
                status: 400,
                body: `Item with id: ${_id} already contains ATA or ATD`
            };
            context.done();
            return;
        };
