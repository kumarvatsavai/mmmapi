...
import { validateTransportsArrival } from '../common/input';
import { TransportArrivalProcessor } from "./TransportArrivalProcessor";
...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    try {
        ...
        const transportProcessor = new TransportArrivalProcessor(apiClient, logger, inputParameters.environment);

        const request = await transportProcessor.getTransport(_id);

        if (request && !request.mmm_ata) {
            const validateFields = validateTransportsArrival(newTransport, context);
            if (validateFields === false) {
                context.done();
                return;
            }
            
            const result = await transportProcessor.updateTransportArrival(_id, newTransport);
            ...
            context.log.warn(`Item with id: ${_id} already contains ATA`);
            context.res = {
                status: 400,
                body: `Item with id: ${_id} already contains ATA`
            };
            context.done();
            return;
        };
