...
import { validateTransportsUnloading } from '../common/input';
import { TransportUnloadingProcessor } from "./TransportUnloadingProcessor";
...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    try {
        ...
        const transportProcessor = new TransportUnloadingProcessor(apiClient, logger, inputParameters.environment);
        const request = await transportProcessor.getTransport(_id);
        const validateFields = validateTransportsUnloading(newTransport, context);
